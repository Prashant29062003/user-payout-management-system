...
describe('WithdrawalService', () => {
  ...
  it('creates a withdrawal with default pending status', async () => {
    ...
    mockAccountRepositoryClass.mockImplementation(() => ({
  ...
  it('returns false when insufficient balance in canWithdraw', async () => {
    const accountRepo = {
      findById: jest.fn().mockResolvedValue({ id: 'acct-1', withdrawableBalance: 50 }),
    };
    const accountRepoClass = jest.fn(() => accountRepo);
    const serviceWithAccountMock = new WithdrawalService(
      mockRepository,
      accountRepoClass,
      mockTransactionRunner
    );
  ...
  it('returns false when a recent withdrawal exists in canWithdraw', async () => {
    const accountRepo = {
      findById: jest.fn().mockResolvedValue({ id: 'acct-1', withdrawableBalance: 150 }),
    };
    const accountRepoClass = jest.fn(() => accountRepo);
    const serviceWithAccountMock = new WithdrawalService(
      mockRepository,
    ...
    mockRepository.findRecentByAccountId.mockResolvedValue([
      { id: 'withdrawal-1', createdAt: new Date() },
    ]);
  ...
  it('returns true when account has sufficient balance and no recent withdrawals', async () => {
    const accountRepo = {
      findById: jest.fn().mockResolvedValue({ id: 'acct-1', withdrawableBalance: 200 }),
    };
    const accountRepoClass = jest.fn(() => accountRepo);
    const serviceWithAccountMock = new WithdrawalService(
      mockRepository,
